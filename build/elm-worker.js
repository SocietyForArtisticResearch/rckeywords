		...
		switch (t) {
			case 'FindKeywords':
				return A4(
					...
					$elm$json$Json$Decode$maybe(
						A2($elm$json$Json$Decode$field, 'mportal', $author$project$Research$decodePortal)));
			case 'FindResearch':
				return A2(
					$elm$json$Json$Decode$map,
...
var $author$project$Queries$filterResearchWithPortal = F2(
...
var $author$project$Worker$findKeywordsPortal = F3(
	function (portal, lst, sorting) {
		return A3(
			$author$project$Worker$findKeywords,
			'',
			sorting,
			$author$project$Research$keywordSet(
				A2($author$project$Queries$filterResearchWithPortal, portal.name, lst)));
	});
		...
		switch (model.$) {
								...
								if (mportal.$ === 'Nothing') {
									...
									return _Utils_Tuple2(
										...
										$author$project$Worker$returnResults(
											$author$project$Queries$encodeSearchResult(
												$author$project$Queries$Keywords(
													A3($author$project$Worker$findKeywordsPortal, portal, lmodel.research, kwSorting)))));
								}
							case 'FindResearch':
								var search = q.a;
								return _Utils_Tuple2(
			...
			default:
				...
				if (msg.$ === 'LoadData') {
					...
					var json = msg.a;
					var _v12 = A2($elm$json$Json$Decode$decodeValue, $author$project$Queries$decodeSearchQuery, json);
					if (_v12.$ === 'Ok') {
						var query = _v12.a;
						return _Utils_Tuple2(
							A2(
