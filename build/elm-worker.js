...
var $author$project$Queries$decodeSearch = A9(
	...
	$elm$json$Json$Decode$maybe(
		A2($elm$json$Json$Decode$field, 'portal', $elm$json$Json$Decode$int)),
	$elm$json$Json$Decode$maybe(
		A2(
			$elm$json$Json$Decode$field,
			'status',
...
var $author$project$Research$encodeDoi = function (_v0) {
	var doi = _v0.a;
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'id',
				$elm$json$Json$Encode$string(doi)),
				_Utils_Tuple2(
				'url',
				$elm$json$Json$Encode$string(
					$author$project$Research$doiUrl(
						$author$project$Research$DOI(doi))))
			]));
};
...
var $author$project$EnrichedResearch$posixToInt = function (posix) {
	var millisToSeconds = function (ms) {
		return $elm$core$Basics$floor(ms / 1000);
	};
...
var $author$project$EnrichedResearch$encodeResearchWithKeywords = function (exp) {
	...
	var lastModified = $elm$json$Json$Encode$int(
		$justinmimbs$date$Date$toRataDie(exp.lastModified));
	...
	var doi = A2(
		...
		function (d) {
			return _Utils_Tuple2(
				'doi',
				$author$project$Research$encodeDoi(d));
		},
		exp.doi);
								...
								A2(
									...
									_List_fromArray(
										[
											...
											_Utils_Tuple2('lastModified', lastModified),
											_Utils_Tuple2(
											'lastModifiedPosix',
											$elm$json$Json$Encode$int(
												$author$project$EnrichedResearch$posixToInt(exp.lastModifiedPosix)))
										])))))))));
};
...
var $author$project$Queries$filterResearchWithPortalID = F2(
	function (portalID, lst) {
...
var $author$project$Worker$findKeywordsPortal = F4(
	function (q, portal, lst, sorting) {
		var _v0 = A2($elm$core$Debug$log, 'q = ', q);
		return A3(
			$author$project$Worker$findKeywords,
			q,
			sorting,
			$author$project$Research$keywordSet(
				A2($author$project$Queries$filterResearchWithPortalID, portal, lst)));
	});
...
var $author$project$Queries$findResearchWithPortal = F2(
	function (mportal, lst) {
		if (mportal.$ === 'Nothing') {
			return lst;
		} else {
			var portalq = mportal.a;
			var f = function (research) {
				var ids = A2(
					$elm$core$List$map,
					function ($) {
		...
		switch (model.$) {
						...
						switch (q.$) {
							case 'FindKeywords':
								...
								if (mportal.$ === 'Nothing') {
									...
									var portal = mportal.a;
									return _Utils_Tuple2(
										$author$project$Worker$Loaded(lmodel),
										$author$project$Worker$returnResults(
											$author$project$Queries$encodeSearchResult(
												$author$project$Queries$Keywords(
													A4($author$project$Worker$findKeywordsPortal, str, portal, lmodel.research, kwSorting)))));
								}
							case 'FindResearch':
								var search = q.a;
								return _Utils_Tuple2(
			...
			default:
				...
				if (msg.$ === 'LoadData') {
								...
								case 'FindKeywords':
									...
									if (mportal.$ === 'Nothing') {
										...
										return $author$project$Worker$returnResults(
											$author$project$Queries$encodeSearchResult(
												$author$project$Queries$Keywords(
													A4($author$project$Worker$findKeywordsPortal, str, portal, data, kwsorting))));
									}
								case 'FindResearch':
									var search = query.a;
					...
					var json = msg.a;
					var _v13 = A2($elm$json$Json$Decode$decodeValue, $author$project$Queries$decodeSearchQuery, json);
					if (_v13.$ === 'Ok') {
						var query = _v13.a;
						return _Utils_Tuple2(
							A2(
