	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				A2($elm$time$Time$every, 1000.0, $author$project$Main$Tick)
			]));
};
												...
												return function (abstractWithKw) {
													return function (simpleToc) {
														return function (screenshots) {
															return {_abstract: _abstract, abstractWithKeywords: abstractWithKw, author: authr, created: created, defaultPage: defaultPage, id: id, issueId: issueId, keywords: keywords, portals: portals, publication: publication, publicationStatus: publicationStatus, screenshots: screenshots, thumbnail: thumbnail, title: title, toc: simpleToc};
														};
													};
												};
											};
	...
	return A2(
		...
		A2(
			...
			A2(
				...
				A2(
					$elm_community$json_extra$Json$Decode$Extra$andMap,
					A2($elm$json$Json$Decode$field, 'abstractWithKeywords', $author$project$EnrichedResearch$decodeAbstractWithKeywords),
					A2(
						$elm_community$json_extra$Json$Decode$Extra$andMap,
						A2(
							$elm$json$Json$Decode$field,
							...
							A2(
								...
								A2(
									...
									A2(
										...
										A2(
											$elm$json$Json$Decode$map,
											$elm$core$Maybe$andThen($author$project$Research$dateFromRCString),
											$elm$json$Json$Decode$maybe(
...
var $author$project$Main$viewExpositionDetails = function (exposition) {
	var images = A2(
		$elm$core$Maybe$withDefault,
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$text('exp has no .screenshots')
			]),
		A2(
			$elm$core$Maybe$map,
			A2(
				$elm$core$Basics$composeR,
				A2($author$project$Screenshots$getUrls, $author$project$Main$screenshotFolder, exposition.id),
				$elm$core$List$map(
					function (url) {
						return A2(
							$mdgriffith$elm_ui$Element$image,
							_List_Nil,
							{description: 'screenshot', src: url});
					})),
			exposition.screenshots));
	return A2($mdgriffith$elm_ui$Element$row, _List_Nil, images);
};
			...
			default:
				...
				switch (_v2.$) {
					case 'FoundResearch':
						...
						return A2(
							$elm$core$Maybe$withDefault,
							$mdgriffith$elm_ui$Element$text('nothing to see'),
							A2(
								$elm$core$Maybe$map,
								$author$project$Main$viewExpositionDetails,
								$elm$core$List$head(
									A2(
										$elm$core$List$filter,
										function (r) {
											return _Utils_eq(r.id, s.id);
										},
										lst))));
					case 'Idle':
						return $mdgriffith$elm_ui$Element$text('idle');
					case 'FoundKeywords':
						return $mdgriffith$elm_ui$Element$text('still keywords');
					default:
						return $mdgriffith$elm_ui$Element$text('working hard');
				}
		}
	}();
	return {
