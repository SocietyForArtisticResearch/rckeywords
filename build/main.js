							...
							return function (pub) {
								return function (publ) {
									return function (thum) {
										return function (abs) {
											return function (d) {
												return function (rcportal) {
													return {_abstract: abs, author: au, created: cr, defaultPage: d, id: i, issueId: iss, keywords: kw, portals: rcportal, publication: publ, publicationStatus: pub, thumbnail: thum, title: t};
												};
											};
										};
									};
...
var $author$project$Main$isSubkeyword = F2(
	function (keywords, index) {
		var kws = $elm$core$Array$fromList(keywords);
		var second = A3(
			$elm$core$Array$slice,
			index + 1,
			$elm$core$Array$length(kws),
			kws);
		var kw = A2(
			$elm$core$Maybe$withDefault,
			'',
			A2($elm$core$Array$get, index, kws));
		var first = A3($elm$core$Array$slice, 0, index, kws);
		var arr = A2($elm$core$Array$append, first, second);
		var bools = A2(
			$elm$core$Array$map,
			$elm$core$String$contains(kw),
			arr);
		var list = $elm$core$Array$toList(bools);
		return A2($elm$core$List$member, true, list);
	});
...
var $author$project$Main$makeSnippet = F5(
	function (indexes, subkeywords, keywords, _abstract, which) {
		var subs = $elm$core$Array$fromList(subkeywords);
		var kwsLength = $elm$core$List$length(keywords);
		var kws = $elm$core$Array$fromList(keywords);
		var isSub = A2(
			$elm$core$Maybe$withDefault,
			false,
			A2($elm$core$Array$get, which, subs));
