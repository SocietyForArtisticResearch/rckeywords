...
var $author$project$Main$splitInColumns = F2(
	function (n, lst) {
		var chunk = function () {
			var _v1 = ($elm$core$List$length(lst) / n) | 0;
			if (!_v1) {
				return 1;
			} else {
				var x = _v1;
		...
		var auxe = function (l) {
			if (!l.b) {
				...
				return A2(
					$elm$core$List$cons,
					A2($elm$core$List$take, chunk, xs),
					auxe(
						A2($elm$core$List$drop, chunk, xs)));
			}
		};
		return auxe(lst);
	});
...
var $author$project$Main$viewList = function (model) {
	...
	var cls = A2($author$project$Main$splitInColumns, 3, filtered);
	var _v0 = A2(
		$elm$core$Debug$log,
		'n=',
		$elm$core$List$length(cls));
	return A2(
